...
from .blocks import MaskedConv1D_dw
...
class PtTransformerClsHead(nn.Module):
    ...
    def __init__(
        ...
        if prior_prob > 0:
            ...
            # if dw:
            #     torch.nn.init.constant_(self.cls_head.conv.depth_conv.bias, bias_value)
            #     torch.nn.init.constant_(self.cls_head.conv.point_conv.bias, bias_value)
            # else:
...
# todo syx
# 看情況是否需要從i3d的rgb直接提取特徵之後，再進行sample操作
class Rgb2Flow(torch.nn.Module):
    def __init__(self):
        super().__init__()
        embed_dim = 1024
        ...
        # 最下面是relu，因爲i3d特徵中最小的是0， 最大的是5左右
...
class ConvGatedMultimodalLayer(nn.Module):
    def __init__(self, size_out=1024):
        super(ConvGatedMultimodalLayer, self).__init__()
        # self.size_in1, self.size_in2, self.size_out = 64, 64, size_out

        self.hidden1 = nn.Conv1d(size_out, size_out, stride=1, kernel_size=3, padding=1, bias=False)
        self.hidden2 = nn.Conv1d(size_out, size_out, stride=1, kernel_size=3, padding=1, bias=False)
    ...
    def forward(self, x1, x2):
        ...
        z = self.sigmoid_f(self.hidden_sigmoid(x.transpose(-1,-2))).transpose(-1,-2)

        # return z.view(z.size()[0], 1) * h1 + (1 - z).view(z.size()[0], 1) * h2
        return torch.cat((h1 * z, (1 - z) * h2), dim=1)
...
class GatedMultimodalLayer(nn.Module):
    def __init__(self,  size_out=1024):
        ...
        self.hidden1 = nn.Linear(size_out, size_out, bias=False)
        self.hidden2 = nn.Linear(size_out, size_out, bias=False)
    ...
    def forward(self, x1, x2):
        ...
        # return z.view(z.size()[0], 1) * h1 + (1 - z).view(z.size()[0], 1) * h2
        return torch.cat((h1*z,(1-z)*h2), dim=-1).transpose(-1,-2)
...
class PtTransformer(nn.Module):
    ...
    def __init__(
        ...
        # todo syx
        self.rgbOnly = rgbOnly
        if self.rgbOnly:
            self.rgb2flow = Rgb2Flow()

        self.gateFusion = gateFusion
        if self.gateFusion:
            # self.gateFusion = ConvGatedMultimodalLayer()
            self.gateFusion = GatedMultimodalLayer()
        ...
        # self.audio_neck = make_neck(
        #     fpn_type,
        #     **{
        #         'in_channels' : [128] * (backbone_arch[-1] + 1),
        #         'out_channel' : 128,
        #         'scale_factor' : scale_factor,
        #         'start_level' : fpn_start_level,
        #         'with_ln' : fpn_with_ln
    ...
    def forward(self, video_list):
        ...
        # batched_inputs, batched_audios = batched_inputs[:,:2048,:], batched_inputs[:,2048:,:]
        start = time.time()
        # todo syx
        if self.rgbOnly:
            # rgb = feat[:, :1024, :]
            # feat[:, 1024:,:] = rgb
            rgb = batched_inputs[:,:1024, :].clone()
            flow_rgb = self.rgb2flow(rgb)
            batched_inputs[:,1024:2048, :] = flow_rgb
        end = time.time()
        rgb_to_flow = end-start

        # todo syx gate fusion后的i3d获取不应该从batched_inputs中获取
        if self.gateFusion:
            before_gate_batched_inputs = batched_inputs.clone()
            batched_inputs = self.gateFusion(batched_inputs[:,:1024,:], batched_inputs[:,1024:2048,:])

        # forward the network (backbone -> neck -> heads)
        feats, masks, dc = self.backbone(batched_inputs, batched_masks)
        fpn_feats, fpn_masks = self.neck(feats, masks)

        # audio_feats, _ = self.audio_backbone(batched_audios, batched_masks)
        # audio_fpn_feats, _ = self.audio_neck(audio_feats, _)
        #
        # t_fpn_feats = [None for i in range(len(fpn_feats))]
        # for i in range(len(fpn_feats)):
        #     t_fpn_feats[i] = torch.cat((fpn_feats[i], audio_fpn_feats[i]), dim=1)
        # fpn_feats = tuple(t_fpn_feats)
        ...
        # print("forward:{}".format(time.time() - start))
        ...
        if self.training:
            ...
            # todo syx
            losses['out_cls_logits'] = out_cls_logits
            losses['out_offsets'] = out_offsets
            losses['stem_out'] = dc['stem_out']
            if self.gateFusion:
                losses['batched_inputs'] = before_gate_batched_inputs
                losses['gate_batched_inputs'] = batched_inputs
            else:
                losses['batched_inputs'] = batched_inputs
            losses['feats'] = feats
            ...
            # print(len(results[0]['segments']))
            total = time.time() - start
            return results, [rgb_to_flow,forward_time,total]

    # todo syx model summary
    def forward_1(self, batched_inputs):
        ...
        # batched_masks = batched_masks[0]
        batched_masks = torch.full((1,1,2304),True)
