...
# todo syx depth wise
class MaskedConv1D_dw(nn.Module):
    """
    Masked 1D convolution. Interface remains the same as Conv1d.
    Only support a sub set of 1d convs
    """
    def __init__(
        ...
        # self.conv = nn.Conv1d(in_channels, out_channels, kernel_size,
        #                       stride, padding, dilation, groups, bias, padding_mode)
        self.conv = DS_Conv1d(in_channels, out_channels, kernel_size,
                              stride, padding, dilation, groups, bias, padding_mode)
...
class DS_Conv1d(nn.Module):
    def __init__(
        ...
        if bias:
            torch.nn.init.constant_(self.depth_conv.bias, 0.)
            torch.nn.init.constant_(self.point_conv.bias, 0.)
...
class ConvBlock_dw(nn.Module):
    ...
    def __init__(
        ...
        if n_out is None:
         ...
         # 1x3 (strided) -> 1x3 (basic block in resnet)
        width = n_embd * expansion_factor
        self.conv1 = MaskedConv1D_dw(
            n_embd, width, kernel_size, n_ds_stride, padding=padding)
        self.conv2 = MaskedConv1D_dw(
            width, n_out, kernel_size, 1, padding=padding)

        # attach downsampling conv op
        if n_ds_stride > 1:
            # 1x1 strided conv (same as resnet)
            self.downsample = MaskedConv1D_dw(n_embd, n_out, 1, n_ds_stride)
        else:
            self.downsample = None
