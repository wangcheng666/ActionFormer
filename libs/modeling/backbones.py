...
from .blocks import (get_sinusoid_encoding, TransformerBlock, MaskedConv1D,
                     ConvBlock, LayerNorm, MaskedConv1D_dw, ConvBlock_dw)


@register_backbone("convTransformer_old")
...
class ConvTransformerBackbone(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        return out_feats, out_masks, {'stem_out':stem_out}
...
class ConvBackbone(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(n_in, (list, tuple)):
            assert isinstance(n_embd, (list, tuple)) and len(n_in) == len(n_embd)
            self.proj = nn.ModuleList([
                MaskedConv1D_dw(c0, c1, 1) for c0, c1 in zip(n_in, n_embd)
            ])
            n_in = n_embd = sum(n_embd)
        else:
            self.proj = None
        ...
        self.stem = nn.ModuleList()
        for idx in range(arch[1]):
            self.stem.append(ConvBlock_dw(n_embd, 3, 1))

        # main branch using convs with pooling
        self.branch = nn.ModuleList()
        for idx in range(arch[2]):
            self.branch.append(ConvBlock_dw(n_embd, 3, self.scale_factor))
